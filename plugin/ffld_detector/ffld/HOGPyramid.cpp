...
HOGPyramid::HOGPyramid(const cv::Mat & image, int padx, int pady, int interval) : padx_(0),
pady_(0), interval_(0)
{
	...
	const int maxScale = ceil(log(min(image.cols, image.rows) / 40.0) / log(2.0) * interval);
...
#pragma omp parallel for
	for (int i = 0; i < interval; ++i) {
		...
		for (int j = 2; i + j * interval <= maxScale; ++j) {
			cv::resize(scaled, scaled, cv::Size(0, 0), 0.5, 0.5);
...
void HOGPyramid::Hog(const cv::Mat & image, Level & level, int padx, int pady, int cellSize)
{
	...
	for (int y = 0; y < height; ++y) {
		const uint8_t * linem = image.ptr<uint8_t>(max(y - 1, 0));
		const uint8_t * line = image.ptr<uint8_t>(y);
		const uint8_t * linep = image.ptr<uint8_t>(min(y + 1, height - 1));
