...
namespace FFLD
{
/// The HOGPyramid class computes and stores the HOG features extracted from an image at
...
public:
	...
	/// @param[in] image The mage of the Scene.
	...
	HOGPyramid(const cv::Mat & image, int padx, int pady, int interval = 5);
...
private:
	...
	static void Hog(const cv::Mat & image, Level & level, int padx = 1, int pady = 1,
					int cellSize = 8);
